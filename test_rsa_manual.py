...
def truth(u, s, x):
    ...
    if u == "say_medium":
        return 1.0 if 4 < w <= 7 else 0.0
    if u == "say_heavy":
        return 1.0 if w > x else 0.0
    return 0.0


Sal = {"say_light": 0.4, "say_medium": 0.35, "say_heavy": 0.25}
C = {"say_light": 0.1, "say_medium": 0.15, "say_heavy": 0.20}
...
def speaker_belief(e, true_state_override=None):
    if e == "ambiguous":
        ...
        if true_state_override is None:
            raise ValueError("Need true_state_override for exact speaker belief")
...
def pragmatic_speaker_given_true_s(s_true):
    ...
    for e in ["exact", "ambiguous"]:
        ...
        def speaker_belief_local(e_local=e):
            return (
                speaker_belief(e_local, true_state_override=s_true)
                if e_local == "exact"
                else speaker_belief(e_local)
            )
...
L1_table_sorted = L1_table.sort_values("P_L1", ascending=False).reset_index(drop=True)
